...
| **2. 매출 이벤트 + 수수료·net** | `payment.completed` 아웃박스(생성 TX 원자 INSERT + poll 발행) 신설, settlement SALE 적재/gross 증분, 요율 기반 fee+VAT+net 산출(compute-on-read). 취소 코어 diff 0 재검증. | SALE-01/02, FEE-01/02 | 🔨 **진행 중** (계획·검증 PASS, 실행 착수) |
