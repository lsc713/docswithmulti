...
- **편 형식(Load 13 실측 레퍼런스와 정확히 동일):**
  1. **Frontmatter:**
     ...
     ---
     ```
  2. **상단 `[!summary] 핵심 한 줄`** — 여러 문장 가능, 반전 beat는 **굵게**, `[[..|별칭]]` 링크.
  3. **`[!info]` 시리즈 블록**(아래 템플릿, 모든 편 동일 — 현재 편도 링크 포함).
  4. `---` 구분선 후 **번호 섹션** `## 1.` `## 2.` …, 마지막 `## 한 줄`(닫는 한 방 + 다음편 훅).
  5. 콜아웃 자유 사용: `[!tip] 원칙`·`[!warning]`·`[!note]`. mermaid·표·LaTeX(`$$..$$`) Load처럼 사용.
- **`[!info] 시리즈` 블록 템플릿(모든 편 동일, 별칭 `|NN` 형식):**
  ```
  ...
  ```
- **위키링크 별칭:** 편 간 링크는 `[[NN.제목|NN편]]` 또는 `|NN`. Load 링크도 `[[13.동기 홉인 줄 알았다|13편]]`.
- **개념은 인라인 최소화** → `[[쿠버네티스 정리]]` 링크. 그 편의 논지에 필요한 만큼만 설명.
- **길이:** Load 편 분량 참고(중편, ~150~250줄). 코드/YAML 인용은 실제 파일에서.
- **목소리:** 1인칭, 간결, 반전 주도, 정직("나도 …할 뻔했다. 측정이 뒤집었다"). 각 편 끝은 다음편 훅.
