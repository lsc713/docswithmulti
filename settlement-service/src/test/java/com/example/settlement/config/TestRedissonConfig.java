...
/**
 * 테스트 전역 RedissonClient mock (RedissonAutoConfiguration 의 실 Redis 접속 대체 — product TestRedissonConfig 복제).
 * 스케줄러 빈 의존성만 충족 — 실 Redis 없이 @SpringBootTest 컨텍스트 기동.
 * getLock().tryLock() → false 로 스텁해, 혹시 @Scheduled 가 발화해도 run() 이 즉시 skip 하도록 한다.
 *
 * <p>@SpringBootTest 컴포넌트 스캔(com.example.settlement) 경로에 있어 별도 @Import 없이 모든 통합테스트에 적용.
 */
@Configuration
