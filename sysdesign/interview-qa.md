...
**Q. 아이템 부분취소가 가능할 때 멱등키의 취약점은?**
```
같은 금액의 연속 부분취소 시 hash가 동일해서
두 번째 요청이 멱등 처리되어 실제 취소가 안 되는 문제가 있습니다.

예시:
  B(90만원) 중 45만원 1차 취소 → hash-aaa
  B(90만원) 중 나머지 45만원 2차 취소 → hash-aaa (동일)
  → 기존 응답 반환 → 실제 취소 안 됨
```

**Q. 어떻게 해결했나?**
```
hash에 현재 PaymentItem.cancelledAmount를 포함했습니다.

1차: B.cancelledAmount=0 → hash = SHA-256(B:45만:0)
2차: B.cancelledAmount=45만 → hash = SHA-256(B:45만:45만)
→ 다른 hash → 신규 처리

추가 DB 조회 없이 이미 검증 목적으로 조회한 데이터를 활용합니다.
```

**Q. hash 생성 시점이 왜 Payment, PaymentItem 조회 이후인가?**
```
cancelledAmount는 DB 상태에 의존하기 때문입니다.
어차피 Payment, PaymentItem 검증이 먼저 필요하므로
이미 조회된 데이터를 활용해서 추가 DB 조회 없이 hash를 생성합니다.
```
