...
public class RetryRouter {
    ...
    /** cancel_restore_dlq.leg — product 레그 식별(관측성, 설계 §수정2 갭④). */
    ...
    /**
     * DLQ 경로 (설계 §수정2). <b>durable 진실 = cancel_restore_dlq 테이블</b> — 먼저 적재+알림한다.
     *
     * <p>그 다음 payment.cancelled.DLQ 토픽 send 는 전송로로 유지하되 <b>best-effort</b>:
     * 이 토픽은 소비자가 0개(설계 §2 갭②)이고 durable 진실은 방금 쓴 테이블이므로,
