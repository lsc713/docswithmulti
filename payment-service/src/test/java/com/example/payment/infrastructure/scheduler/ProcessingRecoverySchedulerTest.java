...
@DisplayName("ProcessingRecoveryScheduler")
class ProcessingRecoverySchedulerTest {

    @Mock RedissonClient redissonClient;
    @Mock ProcessingRecoveryService processingRecoveryService;
    @Mock RLock lock;

    ProcessingRecoveryScheduler scheduler;

    @BeforeEach
    void setUp() {
        scheduler = new ProcessingRecoveryScheduler(redissonClient, processingRecoveryService);
        ReflectionTestUtils.setField(scheduler, "lockKey", "test:lock:processing-recovery");
    ...
    void run_lockAcquired_callsRecoverAllAndUnlocks() throws InterruptedException {
        ...
        verify(processingRecoveryService).recoverAll();
        verify(lock).unlock();
    }
    ...
    void run_lockNotAcquired_skipsRecoverAll() throws InterruptedException {
        ...
        verify(processingRecoveryService, never()).recoverAll();
    ...
    void run_serviceThrows_lockStillReleased() throws InterruptedException {
        when(lock.tryLock(0, 55, TimeUnit.SECONDS)).thenReturn(true);
        when(lock.isHeldByCurrentThread()).thenReturn(true);
        doThrow(new RuntimeException("service error")).when(processingRecoveryService).recoverAll();
