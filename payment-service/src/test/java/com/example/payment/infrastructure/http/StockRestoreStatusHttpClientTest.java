...
class StockRestoreStatusHttpClientTest {
    ...
    void constructorRequiresInspectionRestTemplateQualifier() throws Exception {
        var parameter = StockRestoreStatusHttpClient.class.getConstructors()[0].getParameters()[0];
    ...
    void setUp() {
        ...
        logger = (Logger) LoggerFactory.getLogger(StockRestoreStatusHttpClient.class);
    ...
    void postsExactInspectionContractAndMapsQuantityEvidence() {
        ...
        when(restTemplate.postForEntity(
            eq("http://product-service/internal/cancel-restores/{cancelRequestId}:inspect"),
            any(), eq(StockRestoreStatusHttpClient.InspectResponse.class), (Object[]) any()))
    ...
    void dependencyFailureReturnsUnknown() {
        when(restTemplate.postForEntity(any(String.class), any(),
            eq(StockRestoreStatusHttpClient.InspectResponse.class), (Object[]) any()))
    ...
    void dependencyFailureWarnUsesOnlyBoundedFieldsWithoutThrowableData() {
        ...
        String expandedUrl = "http://product-service/internal/cancel-restores/"
        ...
        assertThat(client.inspect(new StockRestoreStatusPort.Command(
            cancelRequestId,
            "pay_1",
            List.of(new StockRestoreStatusPort.Item(8L, 2)))).status())
    ...
    void observedHttpMetricUsesTemplatedUriWithoutCancelRequestId() {
        ...
        server.expect(requestTo("http://product-service/internal/cancel-restores/"
                + cancelRequestId + ":inspect"))
            .andExpect(method(HttpMethod.POST))
            .andRespond(withSuccess("{\"status\":\"APPLIED\",\"evidence\":[]}",
                MediaType.APPLICATION_JSON));
        var observedClient = new StockRestoreStatusHttpClient(
            observedRestTemplate,
            "http://product-service",
            CircuitBreaker.ofDefaults("stock-observation-test"));

        assertThat(observedClient.inspect(new StockRestoreStatusPort.Command(
            cancelRequestId,
            "pay_1",
            List.of(new StockRestoreStatusPort.Item(8L, 2)))).status())
