package com.example.order.domain.service;

import com.example.order.domain.entity.Order;
import com.example.order.domain.entity.OrderItem;
import com.example.order.domain.exception.OrderItemsMultipleOrdersException;
import com.example.order.domain.exception.OrderOwnershipMismatchException;
import com.example.order.domain.exception.VerifyOrderItemNotFoundException;

import java.util.List;
import java.util.Set;
import java.util.stream.Collectors;

/**
 * 주문 아이템 검증 도메인 판정 (순수 POJO — Spring/JPA 어노테이션·repository 의존 없음).
 *
 * 판정: 요청된 orderItemId 전부 존재 && 전부 단일 order 소속 && order.userId == 요청자.
 * (docs/superpowers/specs/2026-07-31-order-link-design.md §3)
 */
public class OrderItemVerifier {

    public long resolveOrderId(List<Long> requestedIds, List<OrderItem> foundItems) {
        Set<Long> foundIds = foundItems.stream().map(OrderItem::getId).collect(Collectors.toSet());
        List<Long> missingIds = requestedIds.stream().filter(id -> !foundIds.contains(id)).toList();
        if (!missingIds.isEmpty()) {
            throw new VerifyOrderItemNotFoundException(missingIds);
        }

        Set<Long> orderIds = foundItems.stream().map(OrderItem::getOrderId).collect(Collectors.toSet());
        if (orderIds.size() > 1) {
            throw new OrderItemsMultipleOrdersException();
        }

        return foundItems.get(0).getOrderId();
    }

    public void checkOwnership(Order order, long requesterUserId) {
        if (order.getUserId() != requesterUserId) {
            throw new OrderOwnershipMismatchException();
        }
    }
}
