...
test('the production Gallery passes the selected variant and quantity to buy and cart callbacks', async ({ page }) => {
  ...
  const checkoutRow = page.locator('.checkout-table tbody tr')
  await expect(checkoutRow).toContainText('미니멀 울 블레이저')
  await expect(checkoutRow).toContainText('블랙 / M')
  await expect(checkoutRow).toContainText('3')
  await expect(checkoutRow).toContainText('₩149,000')
